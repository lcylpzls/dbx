...
func TestOpen(t *testing.T) {
	ctx := context.Background()
	dsn := os.Getenv("DBX_TEST_PG_DSN")
	if dsn == "" {
		// 本地未提供 PostgreSQL 时,只覆盖包装函数与连接失败路径。
		db, err := Open(ctx, "postgres://dbx:dbx@127.0.0.1:1/dbx_test?sslmode=disable")
		if err == nil {
			db.Close()
			t.Fatal("本地应返回连接错误")
		}
		if !errx.Is(err, dbx.CodeOpenFailed) {
	...
	defer db.Close()
	runBasic(t, db, func(n int) string { return fmt.Sprintf("$%d", n) })
}
